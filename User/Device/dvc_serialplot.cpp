...
extern "C" {
    ...
    void Serialplot_Init(
        ...
        Enum_Serialplot_Data_Type __Serialplot_Data_Type,
        uint8_t __Frame_Header
    ) {
        if (obj) {
            obj->Init(huart, __Serialplot_Rx_Variable_Assignment_Num,
                      __Serialplot_Rx_Variable_Assignment_List,
                      __Serialplot_Data_Type,
                      __Frame_Header);
        }
    }

    int8_t Serialplot_Get_Variable_Index(Class_Serialplot* obj) {
        return obj ? obj->Get_Variable_Index() : -1;
    }

    double Serialplot_Get_Variable_Value(Class_Serialplot* obj) {
        return obj ? obj->Get_Variable_Value() : 0.0;
    }
    ...
    void Serialplot_Set_Data(Class_Serialplot* obj, uint8_t Number, ...) {
        ...
        // 临时存储参数
        const void* params[12];
        for (uint8_t i = 0; i < Number; i++) {
            params[i] = va_arg(args, const void*);
        }
        va_end(args);

        // 调用类方法
        obj->Set_Data(Number, params[0], params[1], params[2], params[3],
